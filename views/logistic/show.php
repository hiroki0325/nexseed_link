<?php
    ...
    $image = "thing_image/".$_SESSION['image'];
...
<html lang="ja">
  ...
  <body>
    <h2>投稿の内容を確認しましょう</h2>
    <ul>
      <li><?php echo $_SESSION['thing']; ?></li>
      <li><?php echo $_SESSION['category']; ?></li>
      <li><?php echo $_SESSION['insentive']; ?></li>
      <li><?php echo $_SESSION['payment']; ?></li>
      <li><?php echo $_SESSION['due']; ?></li>
    ...
    <div>
        <?php if (!empty($_POST)):?>
            <a href="update.php?id=<?php echo $_REQUEST['id']?>">編集を実行</a>
        <?php endif ; ?>
        <a href="delete.php?id=<?php echo $_REQUEST['id']?>" onClick="push(); return false;">削除を実行</a>
        <script type="text/javascript">
          function push(){
            // 「OK」時の処理開始 ＋ 確認ダイアログの表示
            if(window.confirm('この内容で変更します。')){
              location.href = "delete.php?id=<?php echo $_REQUEST['id']?>"; 
            }
            // 「OK」時の処理終了
            // 「キャンセル」時の処理開始
            else{
    ...
    <?php while($comment = mysqli_fetch_assoc($comments)):?>
        ...
        <?php endif; ?>
        [<a href="comment_delete.php?id=<?php echo $comment['id']?>">削除</a>]
    <?php endwhile;?>
  </body>
</html>
